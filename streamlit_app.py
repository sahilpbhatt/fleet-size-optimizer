...
st.markdown('<p class="sub-header">Advanced Value Function Approximation for Crowdsourced Delivery Operations<br>📚 Research Implementation | 🏆 M.S. Thesis Project</p>', unsafe_allow_html=True)
...
with author_col2:
    ...
    <div style='text-align: center; padding: 1rem; background: linear-gradient(135deg, #ffffff 0%, #f8f9ff 100%); border-radius: 12px; box-shadow: 0 2px 12px rgba(0,0,0,0.06);'>
        ...
        <div style='display: flex; justify-content: center; gap: 0.5rem; flex-wrap: wrap;'>
            ...
            <span class='author-badge'>📄 <a href='#' style='text-decoration: none; color: #5B6FED;'>Download Resume</a></span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("ℹ️ About This Platform", expanded=True):
        ...
        **Research Implementation**
        ...
        This platform implements cutting-edge optimization algorithms from my M.S. thesis research, published in *Omega Journal* (2024).
    ...
    prob_enter = st.slider(
        ...
        step=0.1,
        help="Probability that drivers accept work assignments"
    )
    ...
    penalty_type = st.selectbox(
        ...
        help="Mathematical form of constraint violation penalties"
    )
    ...
    <div class='info-box'>
        ...
        • Service Level: <span class='achievement-badge'>95%</span><br>
        • Utilization: <span class='achievement-badge'>80%</span><br>
        • Penalty Coefficient: <strong>250</strong>
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'results' in st.session_state:
        ...
        with col2:
            ...
            fig_util = go.Figure(data=[
                go.Bar(
                    ...
                    marker=dict(
                        color=[d*100 for d in util_dist],
                        colorscale='RdYlGn',
                        showscale=False,
                        line=dict(width=0)
                    ),
            ...
            fig_cost = px.pie(
                ...
                color_discrete_sequence=['#5B6FED', '#EF4444', '#F59E0B', '#10B981']
            )
        ...
        with col1:
            ...
            st.download_button(
                label="📄 Download CSV",
                data=csv,
                file_name=f"fleet_optimization_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                mime="text/csv"
            )
        
        with col2:
            # Excel export placeholder
            st.button("📊 Download Excel", disabled=True, help="Excel export coming soon")
        
        with col3:
            # PDF report placeholder
            st.button("📑 Generate Report", disabled=True, help="PDF report generation coming soon")
...
<div style='background: linear-gradient(120deg, #5B6FED 0%, #7B68EE 100%); padding: 2.5rem; border-radius: 16px; text-align: center; color: white; box-shadow: 0 10px 30px rgba(91,111,237,0.3);'>
    ...
    <div style='display: flex; justify-content: center; gap: 1rem; flex-wrap: wrap; margin-top: 2rem;'>
        <a href='https://github.com/sahilpbhatt' style='background: white; color: #5B6FED; padding: 0.75rem 1.5rem; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-block;'>View More Projects</a>
        <a href='mailto:sahil.bhatt@torontomu.ca' style='background: rgba(255,255,255,0.2); color: white; padding: 0.75rem 1.5rem; border-radius: 8px; text-decoration: none; font-weight: 600; border: 2px solid white; display: inline-block;'>Contact Me</a>
    </div>
</div>
""", unsafe_allow_html=True)
